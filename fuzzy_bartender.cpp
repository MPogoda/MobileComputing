...
FuzzyBartender::FuzzyBartender( const uint8_t level )
    ...
    , time_{ new fl::InputVariable{ "Time" } }
    , groupSize_{ new fl::InputVariable{ "Group" } }
    , drunkness_{ new fl::InputVariable{ "Drunkness" } }
    , beverage_{ new fl::OutputVariable { "Beverage" } }
    , rules_{ new fl::RuleBlock{ "Rules" } }
{
    p_configureTime();
...
{
    drunkness_->setRange(0.000, getDrunknessMaximum( level ) );
    switch( level ) {
    case 0:
